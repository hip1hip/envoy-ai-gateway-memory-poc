...
# Envoy AI Gateway v0.5 migration 검증 환경을 구성한다.
# 기준 환경: WSL2 Ubuntu + Docker Desktop + Kind
# 핵심 목적: v0.5 환경에 v0.4 basic manifest를 먼저 적용해 migration evidence를 확보한다.
...
AI_GATEWAY_VERSION="${AI_GATEWAY_VERSION:-v0.5.0}"
AI_GATEWAY_REPO_URL="${AI_GATEWAY_REPO_URL:-https://github.com/envoyproxy/ai-gateway.git}"
AI_GATEWAY_V05_DIR="${AI_GATEWAY_V05_DIR:-${HOME}/workspace/ai-gateway-v05}"
...
V04_SOURCE_MANIFEST="${V04_SOURCE_MANIFEST:-${REPO_ROOT}/manifests/v04/basic.yaml}"
V05_MIGRATED_MANIFEST="${V05_MIGRATED_MANIFEST:-${REPO_ROOT}/manifests/v05/basic-migrated.yaml}"
...
confirm_cluster_recreate() {
  ...
  echo "v0.5 migration 검증은 ${KIND_NODE_IMAGE} 기준으로 진행합니다."
...
ensure_ai_gateway_example_repo() {
  echo "== Envoy AI Gateway v0.5.0 example repo 준비 =="

  mkdir -p "$(dirname "${AI_GATEWAY_V05_DIR}")"

  if [[ -d "${AI_GATEWAY_V05_DIR}/.git" ]]; then
    echo "기존 repo를 재사용합니다: ${AI_GATEWAY_V05_DIR}"
    git -C "${AI_GATEWAY_V05_DIR}" fetch --tags origin "${AI_GATEWAY_VERSION}"
    git -C "${AI_GATEWAY_V05_DIR}" checkout "${AI_GATEWAY_VERSION}"
  else
    git clone --branch "${AI_GATEWAY_VERSION}" "${AI_GATEWAY_REPO_URL}" "${AI_GATEWAY_V05_DIR}"
  fi
}

apply_v04_source_manifest() {
  echo "== v0.5 환경에 v0.4 source manifest 적용 =="

  if [[ ! -f "${V04_SOURCE_MANIFEST}" ]]; then
    echo "v0.4 source manifest를 찾을 수 없습니다: ${V04_SOURCE_MANIFEST}" >&2
    exit 1
  fi

  kubectl apply -f "${V04_SOURCE_MANIFEST}"
}

apply_v05_migrated_manifest_if_present() {
  echo "== v0.5 migrated manifest 적용 여부 확인 =="

  if [[ -f "${V05_MIGRATED_MANIFEST}" ]]; then
    kubectl apply -f "${V05_MIGRATED_MANIFEST}"
  else
    echo "아직 migrated manifest가 없습니다: ${V05_MIGRATED_MANIFEST}"
    echo "현재 단계에서는 v0.4 source manifest 적용 결과를 migration evidence로 사용합니다."
  fi
}
