...
# Envoy AI Gateway v0.5 migration 검증 상태를 확인하고 수동 검증 명령을 출력한다.
...
EOF
  ...
  print_section "GatewayConfig CRD 확인"
  ...
  if [[ -z "${envoy_service}" ]]; then
    echo "Envoy Service를 찾지 못했습니다. v0.4 source manifest 적용 상태를 확인하세요." >&2
    exit 1
  fi
  ...
  cat <<EOF
kubectl port-forward -n envoy-gateway-system svc/${envoy_service} ${LOCAL_FORWARD_PORT}:80
EOF
...
curl -i \\
  ...
  http://localhost:${LOCAL_FORWARD_PORT}/v1/chat/completions
